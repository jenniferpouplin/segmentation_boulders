...
class Healthy(Dataset):
    def __init__(self, root, transforms=None, mode='train', opts=None):
        ...
        data = pickle_load(join(root, 'boulders.pkl'))
        image, label = data['image'], data['label']
        image, label = shuffle(image, label, random_state=2)

        nb_im = 20 # images per test/ val set
        if mode=='train':
    ...
    def transform(self, image, mask):
        ...
        image, mask = PIL.Image.fromarray(image).convert('L'), PIL.Image.fromarray(mask).convert('L')

        size = 256
        top, left = np.random.random_integers(0, size-1), np.random.random_integers(0, size-1)
        image, mask = TF.crop(image, top, left, size, size), TF.crop(mask, top, left, size, size)
        # Random adjust_gamma
        # if random.random() > 0.5:
        #     image = TF.adjust_gamma(image, gamma=np.random.rand())
        ...
        if random.random() > 0.75:
            # angle_rnd = np.random.randint(-90, 90)
    ...
    def __getitem__(self, index):
        ...
        image, label = self.transform(self.image[index], self.label[index])
        return image, label
    ...
    def __getsize__(self):
        # custom_trans(self.label[0]).size
...
if __name__ == '__main__':
    ...
    testdata = Healthy(mode='test', root='../')
